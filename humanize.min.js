...
Humanize.naturalTimeDelta=function(timestampa,timestampb){timestampa=Humanize.processDate(timestampa);timestampb=Humanize.processDate(timestampb);timedelta=timestampb-timestampa;var oneminute=60;var onehour=60*oneminute;var oneday=onehour*24;var days=(timedelta-timedelta%oneday)/oneday;var tsafterdays=timedelta-days*oneday;var hours=(tsafterdays-tsafterdays%onehour)/onehour;var tsafterhours=tsafterdays-hours*onehour;var minutes=(tsafterhours-tsafterhours%oneminute)/oneminute;var ret="";if(days>0)ret+=
days+" "+Humanize.oneOrElse(days,"p\u00e4iv\u00e4","p\u00e4iv\u00e4\u00e4");if(hours>0){if(ret!="")ret+=", ";ret+=hours+" "+Humanize.oneOrElse(hours,"tunti","tuntia")}if(ret!="")ret+=", ";ret+=minutes+" "+Humanize.oneOrElse(minutes,"minuutti","minuuttia");return ret};
...
10)return"hetkien kuluttua";else return seconds+" sekunnin p\u00e4\u00e4st\u00e4";else if(minutes===1)return"minuutin p\u00e4\u00e4st\u00e4";else return minutes+" minuutin p\u00e4\u00e4st\u00e4"}seconds=Math.round(now-timestamp);minutes=Math.round(seconds/60);if(!minutes)if(seconds<=10)return"nyt";else return seconds+" sekuntia sitten";else if(minutes===1)return"minuutti sitten";else return minutes+" minuuttia sitten"}}return date(format,timestamp)};
Humanize.naturalDateTime=function(timestamp,format){var time=Humanize.naturalTime(timestamp,format);var date=Humanize.naturalDay(timestamp,format);if(date==="t\u00e4n\u00e4\u00e4n"){if(time=="nyt"||time=="hetken kuluttua")return time;return date+", "+time}else return date+" "+time};
...
Humanize.linebreaksbr=function(str){return str.replace(/(\r\n|\n|\r)/gm,"<br />")};Humanize.pluralize=function(number,suffix1,suffix2){var singular="",plural="s";if(suffix2!==undefined){singular=suffix1;plural=suffix2}else if(suffix1!==undefined)plural=suffix1;return parseInt(number)===1?singular:plural};Humanize.oneOrElse=function(value,one,otherthanone){if(value==1)return one;return otherthanone};
Humanize.truncatechars=function(string,length){if(string.length>length)return string.substr(0,length-1)+"\u2026";else return string};Humanize.truncatewords=function(string,length){var array=string.split(" ");var result="";for(var i=0;i<length;i++){if(array[i]===undefined)break;result+=array[i];result+=" "}if(array.length>length)result+="\u2026";return result};